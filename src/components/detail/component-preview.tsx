  ...
  return (
    <MiniFrame>
      <div className="flex flex-wrap justify-center gap-2">
        {["Ctrl", "K", "Save", "Run"].map((item, index) => <motion.span key={item} className="rounded-2xl border border-[#E4E7EF] bg-white px-4 py-2 text-xs font-bold text-text-primary shadow-sm" animate={{ y: index === 1 ? [0, -4, 0] : 0 }} transition={{ duration: 1.8, repeat: Infinity }}>{item}</motion.span>)}
      </div>
    </MiniFrame>
  );
}
...
function PricingPreview({ compact, options }: { compact?: boolean; options?: PreviewProps["pricingOptions"] }) {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -3 }}
    >
      <div className="text-sm font-semibold text-text-primary">{tier}</div>
