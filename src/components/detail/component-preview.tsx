...
export function ComponentPreview({ component, compact = false, viewport = "Desktop", theme = "Light", tableOptions, pricingOptions }: PreviewProps) {
  ...
  const isEmergingTrend = component.tags.includes("2026-trend");
    ...
    <div className={cn("overflow-hidden rounded-2xl bg-white shadow-xl shadow-[#171A2B]/8", options?.bordered !== false && "border border-[#E4E7EF]")}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[520px] text-left text-xs">
          <thead className={cn("text-[#6D7285]", options?.stickyHeader && "sticky top-0 bg-white")}><tr>{["Customer", "Status", "Plan", "Spend"].map((heading) => <th key={heading} className="border-b border-[#EEF0F6] px-4 py-3 font-medium">{heading}{options?.sortable && heading === "Spend" ? " ↓" : ""}</th>)}</tr></thead>
          <tbody>
            {options?.loading ? Array.from({ length: 3 }).map((_, row) => <tr key={row}>{Array.from({ length: 4 }).map((__, cell) => <td key={cell} className="px-4 py-3"><span className="block h-3 animate-pulse rounded-full bg-[#EEF0F6]" /></td>)}</tr>) : rows.slice(0, compact ? 3 : 4).map((row, index) => (
              <tr key={row[0]} className={cn(options?.stripedRows && index % 2 === 1 && "bg-[#F7F8FC]")}>
                <td className={cn("border-b border-[#EEF0F6] px-4 font-medium text-[#171A2B]", rowPadding)}><span className="inline-flex items-center gap-2">{options?.selectable ? <input type="checkbox" aria-label={`Select ${row[0]}`} /> : null}{row[0]}</span></td>
