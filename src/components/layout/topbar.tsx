...
import { cliWorkflowComponent as featuredCatalogComponent } from "@/services/catalog-components";
...
export function Topbar({ onCreate }: { onCreate?: () => void }) {
  ...
  useEffect(() => {
    void getLocalSession().then((payload) => setUser(payload.user)).catch(() => setUser(null));
  }, []);
  ...
  const searchComponents = useMemo(
    () => components.some((component) => component.id === featuredCatalogComponent.id)
      ? components
      : [featuredCatalogComponent, ...components],
    [components],
  );

  const results = useMemo<SearchResult[]>(() => {
    const term = query.trim().toLowerCase();
    const componentResults = searchComponents
      .filter((component) => [
        component.name,
    ...
    const collectionResults = collections
      .filter((collection) => [collection.name, collection.description].join(" ").toLowerCase().includes(term))
      .slice(0, 3)
      .map((collection) => ({
    ...
    const adminResult = isAdmin && "admin workspace painel owner".includes(term)
      ? [{ label: "Admin Studio", meta: "Owner workspace", href: "/vault/admin" }]
      : [];

    return [...adminResult, ...componentResults, ...collectionResults];
  }, [collections, isAdmin, query, searchComponents]);
    ...
    <>
      <motion.header
        className={cn(
          ...
          scrolled
            ? "bg-[#F7F8FC]/95 shadow-[0_8px_24px_rgba(23,26,43,0.045)] backdrop-blur-lg"
            : "bg-[#F7F8FC]/86 backdrop-blur-md",
        )}
      ...
      >
        <motion.button
          ...
          aria-label="Search components, categories, tags and collections"
        >
        ...
        <motion.button className="grid size-10 place-items-center rounded-2xl border border-[#E4E7EF] bg-white text-[#6D7285] shadow-sm" aria-label="Jump to filters" onClick={handleFilters} whileHover={reduceMotion ? undefined : { y: -1 }} whileTap={reduceMotion ? undefined : { scale: 0.94 }} transition={fastMotion}>
          <SlidersHorizontal size={17} aria-hidden />
        </motion.button>
        <Link href="/vault/settings" aria-label="Settings" title="Settings" className="grid size-10 place-items-center rounded-2xl border border-[#E4E7EF] bg-white text-[#6D7285] shadow-sm transition hover:-translate-y-0.5 hover:border-[#C9CDDA] hover:text-[#6366F1]">
          <Settings size={17} aria-hidden />
        </Link>
        {isAdmin ? (
          <Link href="/vault/admin" className="inline-flex min-h-10 items-center gap-2 rounded-2xl border border-[#C9C7FF] bg-[#F7F7FF] px-3 text-sm font-semibold text-[#6366F1] shadow-sm transition hover:-translate-y-0.5 hover:bg-[#EEF0FF]">
            <ShieldCheck size={16} aria-hidden /><span className="hidden md:inline">Admin</span>
          </Link>
        ) : null}
        <motion.button className="group inline-flex min-h-10 items-center gap-2 rounded-2xl bg-[#6366F1] px-4 text-sm font-semibold text-white shadow-md shadow-indigo-200 transition-colors duration-150 hover:bg-[#5558e8]" onClick={() => void handleCreate()} whileHover={reduceMotion ? undefined : { y: -1, scale: 1.006 }} whileTap={reduceMotion ? undefined : { scale: 0.975 }} transition={fastMotion}>
          <Plus size={17} aria-hidden /><span className="hidden sm:inline">New Component</span>
        </motion.button>

        <div className="relative">
          <motion.button className={cn("flex min-h-10 items-center gap-2 rounded-2xl border bg-white px-2 shadow-sm", isAdmin ? "border-[#C9C7FF] ring-2 ring-[#6366F1]/10" : "border-[#E4E7EF]")} aria-label="Account menu" aria-expanded={accountOpen} onClick={() => setAccountOpen((value) => !value)} whileHover={reduceMotion ? undefined : { y: -1 }} whileTap={reduceMotion ? undefined : { scale: 0.96 }} transition={fastMotion}>
      ...
      <AnimatePresence>
        {paletteOpen ? (
          <motion.div
            className="fixed inset-0 z-[100] flex items-start justify-center bg-[#171A2B]/25 px-4 pt-[12vh] backdrop-blur-sm"
          ...
          >
            <motion.div
              role="dialog"
              aria-modal="true"
              aria-label="Search Component Vault"
              className="w-full max-w-2xl overflow-hidden rounded-[28px] border border-[#E4E7EF] bg-white shadow-[0_32px_100px_rgba(23,26,43,0.22)]"
              initial={reduceMotion ? false : { opacity: 0, y: -12, scale: 0.98 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={reduceMotion ? undefined : { opacity: 0, y: -8, scale: 0.985 }}
              transition={{ duration: 0.18, ease: motionEase }}
            >
              <div className="flex items-center gap-3 border-b border-[#E4E7EF] px-4 py-3 sm:px-5">
                <Search size={19} className="shrink-0 text-[#6366F1]" aria-hidden />
                <input
                  ref={inputRef}
                  value={query}
                  ...
                  className="min-w-0 flex-1 bg-transparent py-2 text-base text-text-primary outline-none placeholder:text-[#9A9FB1]"
                  placeholder="Search components, categories, tags or collections..."
                  aria-label="Search"
                />
                {query ? (
                  <button className="rounded-xl px-2 py-1 text-xs font-semibold text-[#6D7285] hover:bg-[#F7F8FC]" onClick={() => setQuery("")}>Clear</button>
                ) : null}
                <button className="grid size-9 shrink-0 place-items-center rounded-xl text-[#6D7285] hover:bg-[#F7F8FC]" onClick={closeSearch} aria-label="Close search">
                  <X size={17} aria-hidden />
                </button>
              </div>

              <div className="max-h-[56vh] overflow-y-auto p-2">
                <div className="flex items-center justify-between px-3 py-2 text-[11px] font-bold uppercase tracking-[0.12em] text-[#9A9FB1]">
                  <span>{query.trim() ? "Search results" : "Components and collections"}</span>
                  ...
                  <div className="space-y-1">
                    {results.map((result, index) => (
                      <button
                        ...
                        className="group flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition-colors hover:bg-[#F7F8FC] focus-visible:bg-[#F7F8FC] focus-visible:outline-none"
                        onClick={() => openResult(result)}
                      >
                        <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-[#EEF0FF] text-[#6366F1]">
                          {index === 0 ? <Check size={16} aria-hidden /> : <Command size={15} aria-hidden />}
                        </span>
                        <span className="min-w-0 flex-1">
                          <span className="block truncate text-sm font-semibold text-text-primary">{result.label}</span>
                          <span className="mt-0.5 block truncate text-xs text-[#8A90A2]">{result.meta}</span>
                        </span>
                        <span className="text-xs font-medium text-[#A0A5B5] opacity-0 transition-opacity group-hover:opacity-100">Open</span>
                      </button>
                    ))}
                  </div>
                ) : (
                  <div className="px-5 py-12 text-center">
                    <div className="mx-auto grid size-11 place-items-center rounded-2xl bg-[#F7F8FC] text-[#8A90A2]">
                      <Search size={18} aria-hidden />
                    </div>
                    <p className="mt-3 text-sm font-semibold text-text-primary">No results found</p>
                    <p className="mt-1 text-sm text-[#8A90A2]">Try a component name, category or tag.</p>
                  </div>
                )}
              </div>

              <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-[#E4E7EF] bg-[#FAFAFC] px-5 py-3 text-[11px] font-medium text-[#8A90A2]">
                <span><kbd className="mr-1 rounded border border-[#DDE0E9] bg-white px-1.5 py-0.5">Enter</kbd> open first result</span>
                <span><kbd className="mr-1 rounded border border-[#DDE0E9] bg-white px-1.5 py-0.5">Esc</kbd> close</span>
                <span className="ml-auto hidden sm:inline">Ctrl / Cmd + K</span>
              </div>
            </motion.div>
          </motion.div>
        ) : null}
